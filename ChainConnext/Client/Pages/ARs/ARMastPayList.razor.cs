...
{
    public partial class ARMastPayList
    {
        [Parameter]
        ...
        List<BD_debtorpayb> bD_Debtorpaybs = new List<BD_debtorpayb>();
        ...
        private async Task PaymentListData()
        {
            ...
            var postBody = new BD_debtorpayb { refno = pDbta.refno, contno = pDbta.contno, UserData = userData };
            var response = await Http.PostAsJsonAsync("BD/DebtorPayBList", postBody);
            ...
            if (Rs != null)
            {
                ...
                if (Rs.Data != null)
                {
                    if (Rs.IsSuccess)
                    {
                        if (Rs.Rows > 0)
                        {
                            bD_Debtorpaybs = Newtonsoft.Json.JsonConvert.DeserializeObject<List<BD_debtorpayb>>(Rs.Data.ToString());
                        }
                    }
                    else
                    {
