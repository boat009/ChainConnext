...
{
    public partial class ContractPaymentReport
    {
        [CascadingParameter]
        ...
        async Task CheckPermission()
        {
            ...
            if (userData.MenuList.Count > 0)
            {
                ...
                var menu = userData.MenuList.Find(x => x.MenuId == 21);
                if (menu != null)
                {
        ...
        async Task Find()
        {
            ...
            if (PayDateFrom != null)
            {
                string RDateF = string.Format(new CultureInfo("en-US", true), "{0:dd/MM/yyyy}", PayDateFrom);
                string RDateT = string.Format(new CultureInfo("en-US", true), "{0:dd/MM/yyyy}", PayDateTo);
                if (BaseShared.CheckCompareDate(Convert.ToDateTime(PayDateFrom.Value.ToShortDateString())
                    , Convert.ToDateTime(PayDateTo.Value.ToShortDateString())))
                {
                    stbdQuery.AppendFormat(" And Convert(Date,dbo.ToEnDate(Mp.PAYDATE),103)=Convert(Date,'{0}',103) ", RDateF);
                }
                else
                {
                    stbdQuery.AppendFormat(" And Convert(Date,dbo.ToEnDate(Mp.PAYDATE),103) Between Convert(Date,'{0}',103) And  Convert(Date,'{1}',103) "
                        , RDateF, RDateT);
                }

                string RDateFTH = string.Format(new CultureInfo("th-TH", true), "{0:dd/MM/yyyy}", PayDateFrom);
                string RDateTTH = string.Format(new CultureInfo("th-TH", true), "{0:dd/MM/yyyy}", PayDateTo);
            ...
            var response = await Http.PostAsJsonAsync("Report/RptContractPayment", Rpt);
                ...
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage pck = new ExcelPackage())
                        {
                            ...
                            pck.SaveAs(ms);
                            await jsRuntime.SaveAs("ReportContractPayment.xlsx", pck.GetAsByteArray());
                        }
                    }
                }
            }
