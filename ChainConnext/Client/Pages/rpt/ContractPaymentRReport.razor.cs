...
{
    public partial class ContractPaymentRReport
    {
        [CascadingParameter]
        ...
        async Task CheckPermission()
        {
            ...
            if (userData.MenuList.Count > 0)
            {
                ...
                var menu = userData.MenuList.Find(x => x.MenuId == 75);
                if (menu != null)
                {
        ...
        async Task Find()
        {
            ...
            var response = await Http.PostAsJsonAsync("Report/RptPayRReport", Rpt);
                ...
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage pck = new ExcelPackage())
                        {
                            ...
                            pck.SaveAs(ms);
                            await jsRuntime.SaveAs("ReportContractPaymentR.xlsx", pck.GetAsByteArray());
                        }
                    }
                }
            }
